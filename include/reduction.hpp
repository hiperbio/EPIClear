...
/* For reducing threadblocks that have blockDim.x = 1 and blockDim.y > 32 */
__inline__ __device__ float blockReduceMin(float val) {
        static __shared__ float shared[32];
        val = warpReduceMin(val);
        if ((threadIdx.y % 32) == 0) shared[threadIdx.y / 32] = val;
        __syncthreads();
        val = (threadIdx.y < (256 / 32)) ? shared[threadIdx.y] : FLT_MAX;
...
/* GPU kernel code for saving indexes (encoded in 64-bit integer) of set of SNPs with best candidate score */
